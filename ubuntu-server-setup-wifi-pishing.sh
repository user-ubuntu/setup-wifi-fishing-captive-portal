...
# ===================================
sudo tee /tmp/wifi-pishing/ubuntu-wifi_free-wifi.sh > /dev/null <<'EOL'
#!/bin/bash
...
# ========================================
sudo hostapd "/tmp/wifi-pishing/conf/wifi-hostapd.conf" &
sudo dhcpd -d -cf "/tmp/wifi-pishing/conf/wifi-dhcpd.conf" wlan0 &
sudo dnsmasq -C "/tmp/wifi-pishing/conf/wifi-dnsmasq.conf" &
sudo lighttpd -D -f "/tmp/wifi-pishing/conf/wifi-lighttpd.conf" &

echo "Press Ctrl+C to stop all services..."
while true; do sleep 1; done
EOL
sudo chmod +x /tmp/wifi-pishing/ubuntu-wifi_free-wifi.sh

if [ $? -eq 0 ]; then
    echo "✅ File : /tmp/wifi-pishing/ubuntu-wifi_free-wifi.sh berhasil dibuat."
else
    echo "❌ ERROR: /tmp/wifi-pishing/ubuntu-wifi_free-wifi.sh gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
echo "💻 sudo bash /tmp/wifi-pishing/ubuntu-wifi_free-wifi.sh"
echo ""
